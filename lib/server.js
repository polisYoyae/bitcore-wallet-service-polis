...
WalletService.prototype.savePreferences = function(opts, cb) {
  ...
  var preferences = [{
    ...
    name: 'unit',
    isValid: function(value) {
      return _.isString(value) && _.contains(['polis'], value.toLowerCase());
    },
  }];
...
WalletService.prototype._selectTxInputs = function(txp, utxosToExclude, cb, isInstantSend) {
  ...
  function select(utxos, cb) {
    ...
    if (totalValueInUtxos < txpAmount) {
      self.logi('Total value in all utxos (' + Utils.formatAmountInPolis(totalValueInUtxos) + ') is insufficient to cover for txp amount (' + Utils.formatAmountInPolis(txpAmount) + ')');
      return cb(Errors.INSUFFICIENT_FUNDS);
    }
    if (netValueInUtxos < txpAmount) {
      self.logi('Value after fees in all utxos (' + Utils.formatAmountInPolis(netValueInUtxos) + ') is insufficient to cover for txp amount (' + Utils.formatAmountInPolis(txpAmount) + ')');
    ...
    self.logi('Big input threshold ' + Utils.formatAmountInPolis(bigInputThreshold));
    ...
    _.each(smallInputs, function(input, i) {
      ...
      //log.debug('The input contributes ' + Utils.formatAmountInPolis(netInputAmount));
      ...
      //log.debug('Tx size: ' + Utils.formatSize(txpSize) + ', Tx fee: ' + Utils.formatAmountInPolis(fee));
      ...
      if (!_.isEmpty(bigInputs)) {
        ...
        if (feeVsAmountRatio > Defaults.UTXO_SELECTION_MAX_FEE_VS_TX_AMOUNT_FACTOR) {
          ...
          //log.debug('Fee/Single-input fee: ' + Utils.formatRatio(feeVsSingleInputFeeRatio) + ' (max: ' + Utils.formatRatio(Defaults.UTXO_SELECTION_MAX_FEE_VS_SINGLE_UTXO_FEE_FACTOR) + ')' + ' loses wrt single-input tx: ' + Utils.formatAmountInPolis((selected.length - 1) * feePerInput));
      ...
      //log.debug('Cumuled total so far: ' + Utils.formatAmountInPolis(total) + ', Net total so far: ' + Utils.formatAmountInPolis(netTotal));

      if (netTotal >= txpAmount) {
        var changeAmount = Math.round(total - txpAmount - fee);
        //log.debug('Tx change: ', Utils.formatAmountInPolis(changeAmount));

        var dustThreshold = Math.max(Defaults.MIN_OUTPUT_AMOUNT, Bitcore.Transaction.DUST_AMOUNT);
        if (changeAmount > 0 && changeAmount <= dustThreshold) {
          //log.debug('Change below dust threshold (' + Utils.formatAmountInPolis(dustThreshold) + '). Incrementing fee to remove change.');
    ...
    if (netTotal < txpAmount) {
      //log.debug('Could not reach Txp total (' + Utils.formatAmountInPolis(txpAmount) + '), still missing: ' + Utils.formatAmountInPolis(txpAmount - netTotal));
  ...
  //log.debug('Selecting inputs for a ' + Utils.formatAmountInPolis(txp.getTotalAmount()) + ' txp');
  ...
  self._getUtxosForCurrentWallet({}, function(err, utxos) {
    ...
    async.whilst(function() {
      ...
      select(candidateUtxos, function(err, selectedInputs, selectedFee) {
        ...
        //log.debug('Fee for this selection: ' + Utils.formatAmountInPolis(fee));
      ...
      if (!err) {
        ...
        self.logi('Successfully built transaction. Total fees: ' + Utils.formatAmountInPolis(txp.fee) + ', total change: ' + Utils.formatAmountInPolis(change));
      } else {
